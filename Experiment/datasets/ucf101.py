...
def find_classes(dir):
    ...
    with open(class_list_file) as f:
        ...
        for line in data:
            ...
            classes.append(line_info[1])
            class_to_idx[line_info[1]] = int(line_info[0])-1
            
    # change logic to read from file
...
def ReadSegmentFlow(path, offsets, new_height, new_width, new_length, is_color, name_pattern):
    ...
    for offset_id in range(len(offsets)):
        offset = offsets[offset_id]
        for length_id in range(1, new_length+1):
            frame_name_x = name_pattern % ( length_id + offset) #vid/x/frame000001.jpg
            frame_path_x = path + "/x/" + frame_name_x
            cv_img_origin_x = cv2.imread(frame_path_x, cv_read_flag)
            frame_name_y = name_pattern % ( length_id + offset)
            frame_path_y = path + "/y/" + frame_name_y #vid/y/frame000001.jpg
